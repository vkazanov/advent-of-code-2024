...
1. one way to speed this up would be to skip directly to the next obstacles once a test
   obstacle run is setup. This is possible because an obstacle + a hit direction
   immediately show what the next obstacle or end of map would be.
