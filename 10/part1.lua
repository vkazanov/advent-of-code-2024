...
local function walk_path(map, pos, path_len, reached)
    -- init recursion
    if not path_len then
        path_len = 0
        reached = {}
    end
    ...
    if this_height == 9 and not reached[pos_key(pos)]then
        reached[pos_key(pos)] = true
    ...
    for _, next_pos in ipairs {up(pos), right(pos), down(pos), left(pos)} do
        ...
        score = score + walk_path(map, next_pos, path_len + 1, reached)
        ::continue::
    end
